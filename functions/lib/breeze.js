 ...
 */
const _people = (state) => ({
    get_people: ({ limit, offset, details, filter_json }) => {
    ...
    get_person_details: ({ person_id }) => state.request(ENDPOINTS.PEOPLE.concat('/', person_id.toString())),
    get_profile_fields: () => state.request(ENDPOINTS.PROFILE_FIELDS),
    get_image: ({ img }) => {
        console.log("Hello World", img);
        return state.request(img);
    }
});
...
*/
const _events = (state) => ({
    get_events: ({ start_date, end_date, category_id, eligible, details, limit }) => {
    ...
    get_event: ({ instance_id, schedule, schedule_direction, schedule_limit, eligible, details }) => {
        /* Retrieve all events for a given date range.
        Args:
...
const _check_in = (state) => ({
    event_check_in: ({ person_id, instance_id }) => {
    ...
    event_check_out: ({ person_id, instance_id }) => {
    ...
    get_event_list: ({ instance_id, details, type }) => {
    ...
    get_event_eligible: ({ instance_id }) => {
...
const _contributions = (state) => ({
    list_contributions: ({ start_date, end_date, person_id, include_family, amount_min, amount_max, method_ids, fund_ids, envelope_number, batches, forms }) => {
    ...
    add_contribution: ({ date, name, person_id, uid, processor, method, funds_json, amount, group, batch_number, batch_name }) => {
    ...
    edit_contribution: ({ payment_id, date, name, person_id, uid, processor, method, funds_json, amount, group, batch_number, batch_name }) => {
    ...
    delete_contribution: ({ payment_id }) => {
 ...
 */
exports.BreezeApi = (state) => {
    if (typeof state['request'] !== 'function') {
        throw new ReferenceError('request' + "() is not defined in state object passed in");
    }
    return Object.assign({}, _people(state), _tags(state), _events(state), _check_in(state), _contributions(state), _pledges(state), _forms(state), _volunteers(state), _account(state));
};
//# sourceMappingURL=breeze.js.map